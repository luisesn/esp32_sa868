...
void WAVFileReader::init(const char *file_name) {
...
void WAVFileReader::getFrames(uint16_t *frames, int number_frames)
{
  int16_t left;
  uint16_t ii;
  // fill the buffer with data from the file wrapping around if necessary
  for (uint16_t i = 0; i < number_frames; i++)
  {
     ii=i*2;
    // if we've reached the end of the file then seek back to the beginning (after the header)
    if (m_file.available() == 0) m_file.seek(44);

    // if we only have one channel duplicate the sample for the right channel
    m_file.read((uint8_t *)(&left), sizeof(int16_t));
    frames[ii] = left + 32768;// we need unsigned bytes for the ADC
    if (m_num_channels != 1) {
      m_file.read((uint8_t *)(&left), sizeof(int16_t)); // read in the next sample to the left channel
      frames[ii + 1] = left + 32768;
    } else {
